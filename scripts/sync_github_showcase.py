...
from sync_utils import read_response, write_text_if_changed
...
def github_json_request(url: str, token: str | None, payload: dict | None = None) -> object:
    ...
    try:
        response_text = read_response(request).decode("utf-8")
    except urllib.error.HTTPError as error:
        raise GitHubSyncError(f"GitHub API request failed for {url}: HTTP {error.code}") from error
    ...
    try:
        ...
        raise GitHubSyncError(f"GitHub returned invalid JSON for {url}") from error
...
def github_request(url: str, token: str | None) -> list[dict]:
    ...
    for page in range(1, 101):
        query["page"] = str(page)
        page_url = urllib.parse.urlunsplit(parts._replace(query=urllib.parse.urlencode(query)))
        data = github_json_request(page_url, token)
        if not isinstance(data, list) or any(not isinstance(item, dict) for item in data):
            raise GitHubSyncError(f"Expected a repository list from {page_url}")
        results.extend(data)
        if len(data) < 100:
            return results
    raise GitHubSyncError("GitHub pagination limit reached; preserving existing showcase")
...
def github_text_request(url: str) -> str:
    ...
    try:
        return read_response(request).decode("utf-8", errors="replace")
    except urllib.error.HTTPError as error:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Every other sync script in this repo takes --dry-run/--verbose. This one
    took no arguments at all and wrote _data/github.yml unconditionally, so a
    stray flag -- or `--help` -- still performed a live API fetch and a commit
    -sized write. Parse properly and honour --dry-run."""
    parser = argparse.ArgumentParser(
        description="Sync the public GitHub showcase into _data/github.yml.",
    )
    parser.add_argument("--dry-run", action="store_true",
                        help="Fetch and render without writing _data/github.yml.")
    parser.add_argument("--verbose", action="store_true",
                        help="Print counts for what was fetched.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if OUTPUT_PATH.is_file():
        ...
        # generated_at describes the last content change, not a daily bot run.
        without_timestamp = lambda value: re.sub(r'^generated_at:.*\n', '', value, flags=re.MULTILINE)
        if without_timestamp(previous) == without_timestamp(yaml_output):
            print("No meaningful GitHub showcase changes detected.")
            return 0
    if args.verbose:
        print(f"repos={len(repos)} starred={len(starred)} "
              f"featured={len(featured_repositories)} spotlight={len(spotlight_repos)}")
    if args.dry_run:
        print(f"Dry run: {OUTPUT_PATH.relative_to(ROOT)} left unchanged "
              f"({len(yaml_output)} bytes would be written).")
        return 0
    write_text_if_changed(OUTPUT_PATH, yaml_output)
